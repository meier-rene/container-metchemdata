######
# kegg_derivatised filler script
######
...
insert_kegg_derivatised () {
 /usr/bin/psql -c "insert into library(library_name,library_id,last_updated,library_link) values ('kegg_derivatised','5',date('1970-01-01'),'http://www.kegg.jp');" -h $POSTGRES_IP -U $POSTGRES_USER -d $POSTGRES_DB
 # check if database exists
 last_updated=$(/usr/bin/psql -c "SELECT last_updated FROM library where library_name='kegg_derivatised';" -h $POSTGRES_IP -U $POSTGRES_USER -qtA -d $POSTGRES_DB)
 # get current modification date
 dbdatesecs=$(date -d $last_updated +%s)
 mostcurrentsecs=$dbdatesecs
 mostcurrent=""
 library_id=$(/usr/bin/psql -c "SELECT library_id FROM library where library_name='kegg_derivatised';" -h $POSTGRES_IP -U $POSTGRES_USER -qtA -d $POSTGRES_DB)
 if [ ! -e /data/${KEGG_DERIVATISED_MIRROR} ]
 then
   echo "/data/${KEGG_DERIVATISED_MIRROR} not found"
   return 1       
 fi
 # loop to check each data file
 unset IFS
 for i in $(ls /data/${KEGG_DERIVATISED_MIRROR} | grep -e "gz$")
 do
  echo "file ${i}"
  # check time stamp of file and database 
  filedatesecs=$(date -r /data/${KEGG_DERIVATISED_MIRROR}/$i +%s)
  filedate=$(date -r /data/${KEGG_DERIVATISED_MIRROR}/$i +%Y-%m-%d)
  ...
  # unzip file
  gunzip -c -k /data/$KEGG_DERIVATISED_MIRROR/$i > /tmp/${filename}.csv
