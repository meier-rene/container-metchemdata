...
# write a single chebi entry as postgres query
write_chebi_entry () {
 line=$1
 ...
 echo "WITH ins_compound AS (
     ...
     VALUES ('${vals[1]}', '${vals[2]}', '${vals[3]}', '${vals[4]}', '${vals[5]}', '${vals[6]}', '${vals[7]}', '${inchikey}') on conflict (inchi_key) do update set monoisotopic_mass='${vals[1]}', molecular_formula='${vals[2]}', smiles='${vals[3]}', inchi='${vals[4]}', inchi_key_1='${vals[5]}', inchi_key_2='${vals[6]}', inchi_key_3='${vals[7]}', inchi_key='${inchikey}'
...
delete_chebi_entries () {
 filename=$1
 library_id=$2
 if [ ! -e /tmp/${filename}.sql ]
 then
  echo "Error in delete_chebi_entries(): /tmp/${filename}.sql not found. Nothing to delete."
  return 1
 fi
 ...
 comm -23 <(for (( c=${ranges[0]}; c<=${ranges[1]}; c++ )); do echo CHEBI:$c; done | sort) <(cut -d"|" -f1 /tmp/${filename}.sql | sort) > /tmp/${filename}.delete
 while read line
 do
...
update_chebi () {
 ...
 exists=$(/usr/bin/psql -c "select 1 from library where library_name='chebi';" -h $POSTGRES_IP -U $POSTGRES_USER -qtA -d $POSTGRES_DB)
 ...
 if [ ! "$exists" == 1 ]
 then 
   echo "library chebi does not exist"
   exit 1
 fi
 ...
 do
  ...
  do
   ...
   write_chebi_entry "$line" "$library_id" "/tmp/${filename}.insert_query"
